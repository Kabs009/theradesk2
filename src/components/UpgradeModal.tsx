...
const UpgradeModal: React.FC<UpgradeModalProps> = ({ onClose, onUpgrade, user }) => {
  ...
  const handlePaystackPayment = () => {
    if (!PAYSTACK_PUBLIC_KEY) {
      addToast("Billing is not configured. Please add VITE_PAYSTACK_PUBLIC_KEY to environment variables.", 'error');
      return;
    }
          ...
          {step === 'pricing' ? (
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div
                  className={`p-8 rounded-3xl border-2 transition-all cursor-pointer ${selectedPlan === 'monthly' ? 'border-indigo-600 bg-indigo-50/30' : 'border-gray-100 bg-white hover:border-indigo-200'
                    }`}
                ...
                <div
                  className={`p-8 rounded-3xl border-2 transition-all cursor-pointer ${selectedPlan === 'annual' ? 'border-indigo-600 bg-indigo-50/30' : 'border-gray-100 bg-white hover:border-indigo-200'
                    }`}
