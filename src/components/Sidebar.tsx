...
const Sidebar: React.FC<Props> = ({ view, setView, user, isMobileMenuOpen = false, onCloseMobileMenu }) => {
  const items = [
  ...
  return (
    <>
      ...
      {isMobileMenuOpen && (
        <div
          className="fixed inset-0 mobile-menu-backdrop z-40 md:hidden animate-fade-in-backdrop"
      ...
      {/* Sidebar */}
      <aside className={`
        w-64 gradient-card border-r border-slate-200/50 flex-col h-full shadow-beautiful backdrop-blur-sm relative overflow-hidden
        md:flex
        ${isMobileMenuOpen ? 'flex fixed inset-y-0 left-0 z-50 animate-slide-in-from-left' : 'hidden'}
      `}>
        {/* Decorative gradient */}
        ...
        <button
          onClick={onCloseMobileMenu}
          className="absolute top-4 right-4 md:hidden w-10 h-10 rounded-xl bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-600 hover:text-slate-900 transition-all duration-300 z-20"
        >
